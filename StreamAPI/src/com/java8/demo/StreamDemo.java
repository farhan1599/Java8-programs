...
//find the "speaker" category from the list using stream and sort them ascending order of the price
		...
		products.stream()
		        .filter(p -> p.getCategory().equalsIgnoreCase("speaker"))
...
//===================================================================================================
		...
		boolean anyAbove1Lakh = products.stream()
...
//===================================================================================================
		...
		List<String> distinctNames = products.stream()
...
//===================================================================================================
		...
		System.out.println("<<< GET A SET OF ALL UNIQUE CATEGORIES >>>");
		
		// Set<String> uniqueCategories = products.stream().map(p ->
		// p.getCategory()).collect(Collectors.toSet());
...
//===================================================================================================
		...
		// Get total price of all products using Stream
...
//===================================================================================================
		...
		// Get average price of all products using Stream

		System.out.println("<<< AVERAGE PRICE OF ALL PRODUCT >>>");

		// Double averagePrice = products.stream().collect(Collectors.averagingDouble(p
		// -> p.getPrice()));
...
//===================================================================================================
		...
		// Convert product list to Map (id → name) using Stream
		
		System.out.println("<<< CONVERTED PRODUCT LIST TO MAP<ID,NAME> >>>");
		
		Map<Integer, String> toMap = products.stream()
				                             .collect(Collectors.toMap(Product::getProductId,
				                              Product::getProductName, 
				                             (existing, incoming) -> existing));// handle duplicate IDs
																																																				
		toMap.forEach((id, name) -> System.out.println(id + " → " + name));

		System.out.println("=".repeat(80));

//===================================================================================================

		// Get comma-separated product names using Stream
		
		System.out.println("<<< PRODUCT NAMES SEPARATED BY COMMA >>>");
...
//===================================================================================================
		...
		// Partition products into two groups based on price > ₹10,000 using Stream
		
        System.out.println("<<< PARTITIONING PRODUCTS INTO TWO GROUPS BASED ON PRICE > 10000 >>>");
        
		Map<Boolean, List<Product>> partitioned = products.stream()
				                                          .collect(Collectors.partitioningBy(p -> p.getPrice() > 10000));
		
		partitioned.forEach((flag, list) -> {
			System.out.println(flag ? "Price > ₹10,000" : "Price ≤ ₹10,000");
			list.forEach(System.out::println);
		});
...
//===================================================================================================
		...
		// Get top 3 most expensive products using Stream
...
//===================================================================================================
		...
		// Skip first 3 products and print remaining using Stream
		
		System.out.println("<<< SKIP FIRST 3 PRODUCTS AND PRINT REMAINING >>>");
		
		List<Product> skipFirst3 = products.stream()
				                           .skip(3)
				                           .collect(Collectors.toList());
		
		skipFirst3.forEach(System.out::println);

		System.out.println("=".repeat(80));

//===================================================================================================

		// Process products in parallel using parallelStream
		
		System.out.println("<<< PRODUCTS IN PARALLEL PROCESSING >>>");
		
		products.parallelStream()
		        .filter(p -> p.getPrice() > 10000)
		        .forEach(p -> System.out.println(p.getProductName() + " processed by " + Thread.currentThread().getName()));
